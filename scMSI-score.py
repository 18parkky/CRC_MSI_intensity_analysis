...
def runscMSI_score( PATH_SCANPY_ADATA, PATH_STR_ALLELE_TABLE, filename, alleleTable_filename, DIR_OUT ):
    ...
    for Identifier, edf in AlleleTable.groupby("Identifier"):
        ...
        if len(edf_o) > 0:
            dict_Identifier_To_MSprofile[Identifier] = [ np.mean(edf_o), np.std(edf_o), len(edf_o) ]
            
    for Identifier in adata.obs['Identifier']:
        try: dict_Identifier_To_MSprofile[Identifier]
        except KeyError: dict_Identifier_To_MSprofile[Identifier]=['N/A', 'N/A', 'N/A']
...
def main():
    ...
    parser.add_argument('-at', '--PATH_STR_ALLELE_TABLE ',      
                        help="PATH to the single-cell Scanpy object *unique identifier for each cell stored at df['Identifier']*", 
                        required=True,
                        )
